...
bool AccelStepper::runSpeed()
{
    ...
    unsigned long time = to_us_since_boot(get_absolute_time());   
...
void AccelStepper::setSpeed(float speed)
{
    if (speed == _speed)
        return;
    speed = MIN(MAX(-_maxSpeed, speed), _maxSpeed);
    if (speed == 0.0)
	_stepInterval = 0;
    else
    {
...
long AccelStepper::stepForward()
{
    ...
    _currentPos += 1;
	step(_currentPos);
	_lastStepTime = to_us_since_boot(get_absolute_time());
    return _currentPos;
}
...
void AccelStepper::setOutputPins(uint8_t mask)
{
    ...
    uint8_t i;
    for (i = 0; i < numpins; i++)
	gpio_put(_pin[i], (mask & (1 << i)) ? (HIGH ^ _pinInverted[i]) : (LOW ^ _pinInverted[i]));
}
...
{   
	...
	gpio_init(_enablePin);
        ...
        gpio_put(_enablePin, LOW ^ _enableInverted);
    }
}
